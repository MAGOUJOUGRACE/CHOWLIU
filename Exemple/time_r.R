...
sim = simul_data(n=10000,d=1400)
W1400 = mutual_information(cov_mat = sim$Cov_mat)

sim = simul_data(n=10000,d=1600)
...
# Function that returns a matrix with V-1 rows and 3 columns
# where each row has an edge of the tree and its weight
visualize = function(parent, weight){
  V = length(parent)
  Mat = matrix(nrow=(V-1), ncol=3)
  for (i in 2:V){
    Mat [(i-1), 1:2] = c(parent[i], i)
    Mat [(i-1), 3] = weight[parent[i], i]
  }
  return(Mat)
}
...
adjT = Prim_max_R(weight_mat = W10)
...
a = microbenchmark(Naive_R(weight_mat = W10), times = 100)
mean(a$time)
...
for (elem in dat){
  ...
  a = microbenchmark(Naive_R(weight_mat = elem), times = 1)
  tnaive=c(tnaive, mean(a$time))
  k = k + 1
  print(k)
}

summary(lm(log(tnaive)~log(vs[1:8])))

plot(log(vs[1:8]), log(tnaive))

topt = c()
k = 0
for (elem in dat){

  b = microbenchmark(Prim_max_R(weight_mat = elem), times = 1)
